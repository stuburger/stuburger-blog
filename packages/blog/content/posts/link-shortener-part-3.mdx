---
title: Building a simple serverless link shortener with SST - Part 3
...
Next we instantiate a new `Config.Parameter`. It takes arguments in the same fashion as the `Function` construct (Scope, Id and Props). In SST, a `Config.Parameter` *provisions* a parameter in AWS Parameter Store. Parameter Store is simply a key/value store where we store configuration that we have access to in lambda functions or in our tests by importing from the `sst/node/config` module. We'll see how to do this in just a moment!
...
A *crucial* point to emphasise is that while we have deployed real resources to AWS, the application logic - as thin as it is - is actually running locally on your computer! But it is being invoked remotely! 
