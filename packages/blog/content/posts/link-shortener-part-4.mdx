...
Our hardcoded `RedirectToURL` isn't particularly good use of a "Parameter". While it does allow us to configure our functions via SSM, it would be much better to store these URLs inside a real database. `Parameters` aren't really a good fit for this use case. They're generally used for configuration for the app itself, not as a place to store what is essentially going to be a user's data! For that we need a database. We're going to be using DynamoDB for our database. Almost any database would do but [DynamoDB](https://docs.aws.amazon.com/dynamodb/?pg=dynamodbt&sec=hs) is a great choice for us because SST provides excellent out-of-the-box support for DynamoDB by way of their `Table` construct. We can use this Table construct to provision a DynamoDB `Table` and `bind` it to our `Function` construct so that we can read and write to the database from our application code.
...
- Update our existing API tests so that we seeds our database before tests are run and is cleaned up any data left behind after a test is finished.
- Update our redirect-hander so that it reads from our database and handles bad requests and not found cases.
- You will API test passing and write some new cases for the Bad Request and Not Found cases.
- In the homework section we'll take the training wheels off and give you the opportunity to use your new knowledge to create `Function` that can be used to shorten links! It goes without saying that you'd want some tests for the link shortener lambda function!

## Provisioning a DynamoDB Table

The first thing we neeed to do is actually _provision_ the `Table` that we're going to be using to store our links. Amend your `sst.config.ts` file so that it looks like this:
...
-     });
      ...
      const handler = new Function(stack, "RedirectHandler", {
        handler: "./redirect-handler.handler",
        url: true,
...
```ts:api.test.ts {7,13-24} showLineNumbers
describe("redirect handler", () => {
  const id = "abcde";
  const original = "https://www.example.com";
...
```ts:sst.config.ts showLineNumbers
```
